...
test('world labels stay above the bottom edge on desktop and mobile',()=>{
  ...
  assert.ok(css.includes('.world-trail{left:2%;right:10%;gap:5px}'));
  assert.match(css,/\.living-world \.teacher-home-avatar\{[^}]*bottom:32%;[^}]*height:62%/);
});
...
test('companion artwork and the new experience are available offline',()=>{
  ...
  for(const asset of ['./adventure-home.css?v=4.31.0','./adventure-home.js?v=4.31.0','./assets/baby-dragon.svg'])assert.ok(sw.includes(`'${asset}'`),asset);
