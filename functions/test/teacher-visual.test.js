...
test('teacher exposes every required visual state',()=>{for(const state of ['idle','greeting','speaking','listening','thinking','explaining','pointing','praising','correcting','celebrating','waiting','paused','uncertain','goodbye'])assert.ok(Visual.STATES.includes(state))});
