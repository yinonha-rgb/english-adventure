(function(root){
  ...
  class InteractiveTeacher{
    ...
    listen({automatic=false,retryAttempt=0,echoAttempt=0}={}){
      ...
      recognition.lang=root.EATeacherProviders?.recognitionLanguage?.({phase:item.type===TYPES.WELCOME?'greeting':'activity',restartAttempt,expectedAnswers:[item.correctAnswer,...(item.acceptedAnswers||[])]})||(item.type===TYPES.WELCOME?'he-IL':'en-US');
