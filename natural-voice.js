(function(root,factory){const api=factory(root);if(typeof module==='object'&&module.exports)module.exports=api;else root.EANaturalVoice=api})(typeof globalThis!=='undefined'?globalThis:this,function(root){
  ...
  function chooseVoice(voices,lang,preferred='',gender='not-specified'){
    ...
    const manual=compatible.find(v=>(v.voiceURI===preferred||v.name===preferred)),matched=targetGender==='not-specified'?null:compatible.find(v=>naturalVoiceGender(v)===targetGender),sameGenderAnyLanguage=targetGender==='not-specified'?null:[...(voices||[])].filter(v=>naturalVoiceGender(v)===targetGender).sort((a,b)=>voiceScore(b,b.lang||lang)-voiceScore(a,a.lang||lang))[0],voice=manual||(targetGender==='not-specified'?available[0]:matched||compatible.find(v=>naturalVoiceGender(v)==='not-specified')||sameGenderAnyLanguage||compatible[0])||null,actual=naturalVoiceGender(voice),crossLanguage=!!voice&&!String(voice.lang||'').toLowerCase().startsWith(String(lang).slice(0,2).toLowerCase()),fallbackReason=!voice?(available.length?'opposite-gender-rejected':'no-language-voice'):crossLanguage?'cross-language-gender-match':actual===targetGender?'matched-gender':actual==='not-specified'?'gender-unknown-gentle-correction':'language-match';
  ...
  function genderPitch(gender,actualGender='not-specified'){const requested=normalizeGender(gender),actual=normalizeGender(actualGender);if(requested==='female')return actual==='female'?1.01:actual==='male'?1.08:1.065;if(requested==='male')return actual==='male'?.98:actual==='female'?.92:.94;return 1}
